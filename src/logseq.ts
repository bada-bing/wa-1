...
export function createTitle(issue: Issue) {
  const key = issue.key;
...
export async function executeLogseqProcedure(issue: Issue) {
  ...
  console.dir({ pageName, jiraIssue });
...
async function createLogseqPage(
  pageName: string,
  jiraIssue: string,
  issue: Issue
) {
  ...
  // todo - project needs to be customized in the template
  const jira = {
    issue: jiraIssue,
