...
describe("Task Metadata Generation", () => {
  describe("generateSlug - creates URL-friendly identifiers for tasks", () => {
    ...
    it("shopping_cart pattern only matches if underscores survive sanitization", () => {
      ...
      const slug = generateSlug("ISSUE-10", "Fix shopping_cart bug");
      expect(slug).toBe("ISSUE-10-fix_shopping-cart_bug");
    });
    ...
    describe("Real-world examples from project", () => {
      it('Jira bug: "ISSUE-123" + "Fix checkout button not responding on mobile"', () => {
        const slug = generateSlug(
          "ISSUE-123",
          "Fix checkout button not responding on mobile"
        );
        expect(slug).toBe(
          "ISSUE-123-fix_checkout_button_not_responding_on_mobile"
        );
      });
  ...
  describe("sanitizeSummary - cleans text for use in file names and branches", () => {
    ...
    it("custom replacement only works if pattern survives initial sanitization", () => {
      ...
      expect(sanitizeSummary("Fix shopping_cart issue")).toBe(
        "fix_shopping-cart_issue"
      );
    });

    it('removes "_the_" as noise word after sanitization', () => {
      // "_the_" pattern is replaced with "-" after initial sanitization
      expect(sanitizeSummary("Update the configuration")).toBe(
    ...
    describe("Edge cases", () => {
      ...
      it("handles mixed special characters and text", () => {
        expect(sanitizeSummary("Update UI/UX @user profile!")).toBe(
          "update_ui-ux_-user_profile"
        );
      });
    ...
    describe("Common patterns in Jira summaries", () => {
      it('handles ticket format: "[Component] Fix bug"', () => {
        expect(sanitizeSummary("[WPC] Fix checkout bug")).toBe(
          "wpc-_fix_checkout_bug"
        );
      });
      ...
      it('handles email-like references: "Update user@example.com permissions"', () => {
        expect(sanitizeSummary("Update user@example.com permissions")).toBe(
          "update_user-example-com_permissions"
        );
      });
    });
  });
